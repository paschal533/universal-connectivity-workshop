...
def check_output():
    ...
    try:
        ...
        # Check for ping service creation
        if "Ping service created" not in output:
            print("✗ Missing 'Ping service created' message")
            print("ℹ Make sure you create the ping service with ping.NewPingService(h)")
            return False
        ...
        try:
            # Parse RTT value
            if "ms" in rtt_str:
                rtt_ms = float(rtt_str.replace("ms", ""))
                if rtt_ms > 10000:  # More than 10 seconds
                    print(f"⚠ Warning: RTT seems very high ({rtt_str})")
            elif "s" in rtt_str and "ms" not in rtt_str:
                rtt_s = float(rtt_str.replace("s", ""))
                if rtt_s > 10:
                    print(f"⚠ Warning: RTT seems very high ({rtt_str})")
        except ValueError:
...
def check_code_structure():
    ...
    try:
        ...
        required_imports = [
            ...
            "ping",  # Could be full path or aliased
        ]

        for imp in required_imports:
            if imp not in code:
                print(f"✗ Missing import: {imp}")
                print("ℹ Add: github.com/libp2p/go-libp2p/p2p/protocol/ping")
                return False
        ...
        if "ping.NewPingService" not in code:
            ...
            print("ℹ Add: pingService := ping.NewPingService(h)")
            return False
        print("✓ Ping service creation found")

        # Check for ping call
        if ".Ping(" not in code and "pingService.Ping" not in code:
            print("✗ Missing ping call")
            print("ℹ Add: resultChan := pingService.Ping(ctx, peerID)")
            return False
        print("✓ Ping call found")

        # Check for result handling
        if "result" not in code.lower() or "rtt" not in code.lower():
            print("✗ Missing result/RTT handling")
            print("ℹ Add: result := <-resultChan; fmt.Printf(\"RTT = %s\", result.RTT)")
            return False
        print("✓ Result handling found")
        ...
        # Check for timeout/context usage
        if "context.WithTimeout" in code or "time.Second" in code:
...
def main():
    ...
    try:
        ...
        if not check_code_structure():
            ...
            print("\nQuick fix checklist:")
            print("  1. Import: github.com/libp2p/go-libp2p/p2p/protocol/ping")
            print("  2. Create: pingService := ping.NewPingService(h)")
            print("  3. Ping: resultChan := pingService.Ping(ctx, peerID)")
            print("  4. Handle: result := <-resultChan")
            print("  5. Display: fmt.Printf(\"RTT = %s\", result.RTT)")
            return False
        ...
        if not check_output():
            ...
            print("  • Ensure ping service was created successfully")
